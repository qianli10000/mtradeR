...
JMR_Tab<-function(otu_tab,long_design,logistic_design,outcome,long_idset,logistic_idset,rand.var,
...
dist.abun <- as.matrix(vegdist(otu_tab,method = "bray"))
dist.pres <- as.matrix(vegdist(otu_tab,method = "bray",binary = T))
...
if(length(tune)>1){
  ...
  cv=NULL
  for(rho in tune){
    cv_val=cv.JMR(otu_tab,otu_id,long_design,logistic_design,outcome,long_idset,logistic_idset,rand.var,shrinkage=rho)
    #cat(cv_val,'\n')
    cv=rbind(cv,cv_val)
  }
...
for(i in 1:n_otu){
  ...
  set.seed(321)
  cvfit.abun = cv.glmnet(others_abun, taxa_tr,alpha=0.05,nfolds = 10,family='gaussian')
  ...
  set.seed(321)
  cvfit.pres = cv.glmnet(others_pres, taxa_bin,alpha=0.05,nfolds = 10,family='binomial')
  ...
  res=JMR(taxa,others_abun_input,others_pres_input,long_design,logistic_design,outcome,long_idset,logistic_idset,rand.var,shrinkage,trace=F)
