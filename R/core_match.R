...
core_match<-function(otu,longi_design_all,logistic_design_all,outcome,longi_idset,logistic_idset,rand.var,shrinkage,trace=T){
  ...
  par.ini=c(composition_cov=rep(0,longi_dim),composition_lambda=0,composition_gamma=0,presence_cov=rep(0,longi_dim),presence_lambda=0,presence_gamma=0,outcome_cov=rep(0,logistic_dim),dispersion=1,variance_a=1,variance_b=1)
  names(par.ini)=c(paste('nzAbundance',colnames(longi_design_all),sep =':'), c('nzAbundance_sbj','nzAbundance_set'),
                   paste('Presence',colnames(longi_design_all),sep = ':'), c('Presence_sbj','Presence_set'),
